...
本项目基于 **NovelMart 电商数据集**构建完整的**电商经营分析平台**，覆盖数据探查、存储、清洗、分析到可视化的全流程。数据集包含 **30万+条** 业务记录，时间跨度 2024-01 ~ 2026-06（2.5年），涉及用户、商品、订单、订单明细、评论五大业务模块。
...
| order_items | 194,250 | 订单明细、购买数量、折扣 |
| reviews | 35,000 | 商品评论、评分、认证购买标识 |
| **总计** | **301,250** | **跨2.5年的电商运营数据** |

### 📐 指标口径说明

- **有效订单**：`已完成 / 待发货 / 已发货`（已付款且未取消、未退款）
- **营收 / 实付金额 / 客单价 / 复购率 / RFM / 留存 / 流失**：均基于有效订单计算
- 待付款、已取消、退货中、已退款订单不计入收入，只用于订单状态分布和退款/取消分析
...
1. **用户分层**: 钻石会员(7%)人均消费约2,354元，是普通会员(765元)的3.1倍，贡献约16%的总消费额
2. **地域分布**: 北京/上海/广东/浙江四大核心区域合计贡献约39%的订单
3. **支付偏好**: 支付宝(34.5%)和微信支付(30.3%)占据主导
4. **评分分布**: 4-5星好评评论占比66.2%，平均评分3.82
5. **客单价**: 有效订单平均客单价约314元（低价日用品购买频次高、高价大件占比低，符合电商结构）
6. **复购率**: 全部用户中复购率(>1单)达81.3%（有有效订单用户中达85.5%）
7. **品类表现**: 七大品类GMV占比较均衡（电子17.0%、家居16.1%、母婴15.3%、服装13.7%等），电子与家居合计约33.1%
8. **配送偏好**: 普通快递占比50.2%，加急(24.9%)与当日达(9.9%)合计约34.8%
...
>
> 基于NovelMart 电商数据集（30万+条，2024-2026）独立构建了完整的电商经营分析平台。设计并实现了5张业务数据表(用户/商品/订单/明细/评论)的MySQL数据库，编写了25个优化索引、3个分析视图及RFM存储过程，覆盖CRUD全场景操作。使用Pandas/NumPy完成数据清洗流水线(缺失值处理、异常值检测、特征工程、跨表口径对账)，并进行RFM客户分群、同期群留存分析及商品关联规则挖掘等多维度深度分析。使用Matplotlib产出12张专业数据图表，使用Tableau设计4个交互式业务看板(销售总览/用户分析/商品分析/评论分析)。基于分析结果提炼8条可落地的商业洞察，包括用户价值分层、地域销售策略、品类优化建议等。
