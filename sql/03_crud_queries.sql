...
-- 动态参考日期：取订单表最大日期，避免硬编码日期导致后续数据更新后口径失真
...
SELECT
    ...
    DATEDIFF(@ref_date, u.last_order_date) AS 距今天数
FROM users u
...
SELECT
    ...
    ROUND(p.sales_count / NULLIF(DATEDIFF(@ref_date, p.listing_date), 0), 2)
                                          AS 日均销量,
    CASE
        WHEN p.stock_quantity = 0 THEN '0（紧急补货！）'
        WHEN ROUND(p.stock_quantity / NULLIF(
            p.sales_count / NULLIF(DATEDIFF(@ref_date, p.listing_date), 0), 0), 0) <= 7
            THEN '7天以内（需补货）'
        WHEN ROUND(p.stock_quantity / NULLIF(
            p.sales_count / NULLIF(DATEDIFF(@ref_date, p.listing_date), 0), 0), 0) <= 30
            THEN '30天以内（预警）'
        ELSE '充足'
    END                                    AS 库存预警等级,
    ROUND(p.price - p.cost_price, 2)       AS 单品毛利
...
SELECT
    ...
    u.membership_level                     AS 会员等级,
    -- R: Recency —— 距最后消费天数
    DATEDIFF(@ref_date, MAX(o.order_date))
                                           AS 最近消费距今天数,
    -- F: Frequency —— 历史订单总数
    ...
    -- 综合分层
    CASE
        WHEN (CASE WHEN DATEDIFF(@ref_date, MAX(o.order_date)) <= 30  THEN 5
                    WHEN DATEDIFF(@ref_date, MAX(o.order_date)) <= 90  THEN 4
                    WHEN DATEDIFF(@ref_date, MAX(o.order_date)) <= 180 THEN 3
                    WHEN DATEDIFF(@ref_date, MAX(o.order_date)) <= 365 THEN 2
                    ELSE 1 END
...
SELECT
    ...
    DATEDIFF(@ref_date, listing_date) AS 上架天数,
    ROUND(price * 0.8, 2)               AS 折后价格
FROM products
WHERE DATEDIFF(@ref_date, listing_date) > 180
  AND sales_count < 10
  AND status = '在售'
...
WHERE order_status = '已取消'
  AND order_date < DATE_SUB(@ref_date, INTERVAL 1 YEAR)
UNION ALL
...
WHERE total_orders = 0
  AND registration_date < DATE_SUB(@ref_date, INTERVAL 2 YEAR);

-- 抽样查看几个待删除用户
SELECT user_id, username, real_name, registration_date,
       DATEDIFF(@ref_date, registration_date) AS 注册天数,
