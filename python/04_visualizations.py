...
def chart_monthly_revenue(orders):
    """Line chart of actual_amount summed by month."""
    df = orders.copy()
...
def chart_category_sales(order_items, products):
    """Horizontal bar chart: sales by category, sorted descending."""
...
def export_province_sales(orders):
    """Aggregate sales by province and export as CSV for Tableau."""
...
def chart_top_cities(orders):
    """Bar chart of top 10 cities by total revenue."""
...
def chart_weekly_pattern(orders):
    """Bar chart of order count by day of week (Monday-Sunday)."""
    df = orders.copy()
...
def main():
    ...
    chart_category_sales(data['order_items'], data['products'])
