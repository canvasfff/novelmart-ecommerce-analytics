...
print("\n  --- 3.1 各会员等级平均消费 ---")
membership_spending = orders_with_users.groupby('membership_level').agg(
    订单数=('order_id', 'count'),
...
print("\n  --- 3.2 各省份消费统计 (Top 15) ---")
province_spending = orders_with_users.groupby('shipping_province').agg(
    订单数=('order_id', 'count'),
...
if 'day_of_week' in orders.columns:
    day_order_stats = orders.groupby('day_of_week').agg(
        订单数=('order_id', 'count'),
...
print("\n  --- 4.1 月度营收趋势 ---")
monthly_revenue = orders.groupby('order_year_month').agg(
    订单数=('order_id', 'count'),
...
print("\n  --- 4.2 月度订单量趋势 ---")
monthly_orders = orders.groupby('order_year_month').size()
...
print("\n  --- 5.3 Top 10 城市 (按订单数) ---")
city_orders = orders.groupby(['shipping_province', 'shipping_city']).agg(
    订单数=('order_id', 'count'),
...
avg_order_value = orders['actual_amount'].mean()
...
# 发现9：星期效应
if 'day_of_week' in orders.columns:
    weekday_avg = orders[orders['day_of_week'].isin([0,1,2,3,4])]['actual_amount'].mean()
    weekend_avg = orders[orders['day_of_week'].isin([5,6])]['actual_amount'].mean()
